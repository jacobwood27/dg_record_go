...
func interpolateGPX(ts timestamps, locs locstamps) Stamps {
	var tls []Stamp
...
func GetRoundRaw(ts_csv string, rec_gpx string) (Stamps, string) {
	ts := parseTimestampFile(ts_csv)

	gpx := parseGPXFile(rec_gpx)
